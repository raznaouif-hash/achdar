...
async function initDB() {
    ...
    // Migrate category: Chauffage → Climatisation
    await db.query(`UPDATE incidents SET category = 'Climatisation' WHERE category = 'Chauffage';`);

    // Add archived column if it doesn't exist yet (safe migration)
    await db.query(`
        ALTER TABLE incidents ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT false;
    `);

    // Add cost column if it doesn't exist yet (safe migration)
    await db.query(`
        ALTER TABLE incidents ADD COLUMN IF NOT EXISTS cost DECIMAL(10,2) DEFAULT NULL;
    `);

    // Add whatsapp column if it doesn't exist yet (safe migration)
    await db.query(`
        ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp VARCHAR(20) DEFAULT NULL;
    `);

    // Set WhatsApp numbers for known users
    await db.query(`
        UPDATE users SET whatsapp = '+33664095629'  WHERE id = 1;
        UPDATE users SET whatsapp = '+212770070409' WHERE id = 2;
        UPDATE users SET whatsapp = '+212770070409' WHERE id = 3;
        UPDATE users SET whatsapp = '+212666798755' WHERE id = 4;
    `);
